...
K_TIMER_DEFINE(time_trigger, timeout_handler, NULL);
...
void uart_task(void *unused1, void *unused2, void *unused3)
{
    ...
    while (true) {
        ...
        if (uart_poll_in(uart_dev, &rc) == 0) {
            ...
            // Jos vastaanotettu merkki ei ole rivinvaihto, lisätään puskurin loppuun
            if (rc != '\r') {
                ...
                if (strlen(uart_msg) == 6) {
                    ...
                    if (delay_s >= 0) {
                        ...
                        printk("Invalid time string: %s (error %d)\n", uart_msg, delay_s);
                    }
                }

                // Lähetetään dispatcherille
...
static void timeout_handler(struct k_timer *timer)
{
    printk("Timer triggered -> red signal\n");
    k_mutex_lock(&red_mutex, K_FOREVER);
    k_condvar_signal(&red_signal);
    k_mutex_unlock(&red_mutex);
}
